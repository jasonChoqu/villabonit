  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        ...
        setBannerUrl(`${baseUrl}/${response.data.filter((item: any) => item.id === 3)[0].image}`);
      } catch {
      ...
      <section className="relative h-screen overflow-hidden">
        <div className="absolute inset-0 group overflow-hidden">
          <img
            ...
            src={`${import.meta.env.VITE_API_URL?.replace('/api', '') || window.location.origin}/assets/banners/services_default.png`}
            alt="Hero image"
          />
