...
import type { IClient as IItemResource } from "@/core/types/IClients";
...
const columns = [
  ...
  },
  {
    key: "name",
    header: "Nombre",
    render: (item: IItemResource) => <div className="font-bold">{item.name}</div>,
    sortable: true,
  },
  {
    key: "email",
    header: "Email",
    render: (item: IItemResource) => <span className="">{item.email}</span>,
    sortable: true,
  },
  {
    key: "phone",
  ...
  },
  {
    key: "address",
    header: "Dirección",
    render: (item: IItemResource) => <div className="flex flex-col">{item.address}</div>,
  },
];
...
export default function ClientsList() {
  ...
  const confirmDelete = (item: IItemResource) => {
    openDialog(
      ...
      `¿Estás seguro que deseas eliminar al cliente ${item.name}?`,
      () => handleDelete(item),
      "danger"
    );
  };
  ...
  const actions = [
    {
      ...
      show: (item: IItemResource) => item.id && hasPermission("cliente_editar"),
    },
    {
      label: "Eliminar",
      ...
      show: (item: IItemResource) => item.id && hasPermission("cliente_eliminar"),
    },
  ];
  ...
  const renderToolbar = () => (
    <div className="flex flex-col gap-4 w-full sm:flex-row sm:items-center sm:justify-between">
      <div className="flex gap-2">
        <WithPermission permissions={["cliente_crear"]}>
          <button
