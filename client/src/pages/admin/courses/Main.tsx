...
import type { ICourse as IItemResource } from "@/core/types/ICourse";
...
const columns = [
  ...
  },
  {
    ...
    header: "Instructor",
    render: (item: IItemResource) => (
      <div>{item.instructor_id || 'Por asignar'}</div>
    ),
    sortable: true,
  },
  {
  ...
  },
  {
    ...
    render: (item: IItemResource) => (
      <span className="badge badge-info">
        {item.modality === 'virtual' ? 'Virtual' : 'Presencial'}
      </span>
    ),
    sortable: true,
  },
  {
    ...
    header: "Ubicación",
    render: (item: IItemResource) => (
      <div>{item.location || (item.modality === 'virtual' ? 'Online' : 'Por definir')}</div>
    ),
  },
  {
    key: "cost",
    ...
    render: (item: IItemResource) => (
      <div className="font-bold">
        <span className={`badge ${item.status === 'active' ? 'badge-success' :
            item.status === 'full' ? 'badge-warning' : 'badge-secondary'
          }`}>
          {item.status === 'active' ? 'Disponible' :
            item.status === 'full' ? 'Completo' : 'Inactivo'}
        </span>
      </div>
    ),
    sortable: true,
  },
  {
    ...
    header: "Certificado",
    render: (item: IItemResource) => (
      <span className={`badge ${item.has_certificate ? 'badge-success' : 'badge-secondary'
        }`}>
        {item.has_certificate ? 'Sí' : 'No'}
      </span>
    ),
    sortable: true,
  },
  {
    key: "attendance",
    header: "Control Asistencia",
    render: (item: IItemResource) => (
      <span className={`badge ${item.attendance_tracking ? 'badge-primary' : 'badge-secondary'
        }`}>
        {item.attendance_tracking ? 'Si' : 'No'}
      </span>
    ),
    sortable: true,
  }
...
export default function CoursetList() {
  ...
  const confirmDelete = (item: IItemResource) => {
    openDialog(
      ...
      `¿Estás seguro que deseas eliminar el curso ${item.name}?`,
      () => handleDelete(item),
      "danger"
    );
  };
  ...
  const actions = [
    {
      ...
      show: (item: IItemResource) =>
        item.id && hasPermission("curso_editar"),
    },
    {
      label: "Eliminar",
      ...
      show: (item: IItemResource) =>
        item.id && hasPermission("curso_eliminar"),
    },
  ];
  ...
  const renderToolbar = () => (
    <div className="flex flex-col gap-4 w-full sm:flex-row sm:items-center sm:justify-between">
      <div className="flex gap-2">
        <WithPermission permissions={["curso_crear"]}>
          <button
