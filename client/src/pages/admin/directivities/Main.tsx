...
import type { IDirectivity as IItemResource } from "@/core/types/IDirectivity";
...
export default function DirectivityList() {
  const {
    items,
    loading,
    pagination,
  ...
  const confirmDelete = (item: IItemResource) => {
    openDialog(
      ...
      `¿Estás seguro que deseas eliminar la directiva ${item.name}?`,
      () => handleDelete(item),
      "danger"
    );
  };
  ...
  const handleDelete = async (item: IItemResource) => {
    try {
      ...
      console.error("Error al eliminar la directiva:", error);
    } finally {
  ...
  const actions = [
    {
      ...
      show: (item: IItemResource) =>
        item.id && hasPermission("directiva_editar"),
    },
    {
      label: "Eliminar",
      ...
      show: (item: IItemResource) =>
        item.id && hasPermission("directiva_eliminar"),
    },
  ];
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Directiva" />
      <DataTable
