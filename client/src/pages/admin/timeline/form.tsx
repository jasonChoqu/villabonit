...
import { MoralValueStoreSchema as storeSchema, MoralValueUpdateSchema as updateSchema } from "./validation";
...
const MoralValuesModal = ({ isOpen, onClose, initialData = null, load }: MoralValuesModalProps) => {
  ...
  const handleSubmit = async (data: FormValues) => {
    ...
    if (isEditing) {
      await ItemService.update(initialData!.id, cleanData as IUpdateRequest)
        .then((response) => {
          toastify.success(response.message || "Elemento de línea de tiempo actualizado");
          onClose();
          load();
        })
      ...
      await ItemService.create(cleanData as ICreateRequest)
        .then((response) => {
          toastify.success(response.message || "Elemento de línea de tiempo creado");
          onClose();
          load();
        })
        .catch((error) => toastify.error(error.response.data.message));
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? "Editar Elemento" : "Nuevo Elemento"} size="lg">
      <FormProviderWrapper
      ...
      >
        <div className="grid grid-cols-1 gap-6">
          <div className="col-span-1">
            <InputField name="title" label="Título" placeholder="Ej: Respeto, Honestidad, Responsabilidad..." />
          </div>
          ...
          <div className="col-span-1">
            <TextAreaField
              name="description"
              label="Descripción"
              placeholder="Describa el elemento de línea de tiempo y su importancia..."
              rows={6}
            />
          </div>
        </div>
